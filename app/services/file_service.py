...
class FileService:
    ...
    def detect_csv_encoding_and_delimiter(
        ...
        for enc in encodings_to_try:
            try:
                with open(filepath, "r", encoding=enc) as f_csv:
                ...
                logger.error(f"Error trying encoding '{enc}': {e}")
                continue
        
        return detected_encoding, detected_delimiter, headers
    
    def suggest_email_column(self, headers: List[str]) -> Optional[str]:
    ...
    def extract_emails_from_csv(
        ...
        emails = []
        with open(filepath, "r", encoding=encoding) as f_csv:
    ...
    def detect_txt_encoding(self, filepath: Path) -> Optional[str]:
