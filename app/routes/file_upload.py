...
def load_csv():
    ...
    try:
        ...
        try:
            ...
            logger.exception("API /load_csv: Error processing CSV")
        ...
        logger.exception("API /load_csv: Unexpected error")
        with state.lock:
            state.set("status", "error")
            state.set("error_message", str(e))
...
def load_txt():
    ...
    try:
        ...
        try:
            ...
            logger.exception("API /load_txt: Error processing TXT")
            if "uploaded_filepath" in locals() and uploaded_filepath.exists():
                uploaded_filepath.unlink()
            with state.lock:
                state.set("status", "error")
        ...
        logger.exception("API /load_txt: Unexpected error")
        with state.lock:
            state.set("status", "error")
            state.set("error_message", str(e))
        return jsonify({"error": "Chyba při zpracování TXT."}), 500
    ...
    if file_type == "txt":
        ...
        try:
            ...
            with state.lock:
                state.set("status", "error")
                state.set("error_message", str(e))
            logger.exception("API /select_column: Error extracting emails")
...
def download_results():
    ...
    try:
        ...
        logger.exception("Error downloading results")
